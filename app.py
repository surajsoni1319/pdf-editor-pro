...
if feature == "✍️ Sign PDF":
    ...
    if pdf_file and sig_file:
        ...
        # Show download section if button is clicked
        if st.button("🔄 Ready to Download? Click Here", type="secondary", use_container_width=True):
        ...
        if st.session_state.get('show_download', False):
            if st.session_state.signatures_applied and isinstance(st.session_state.signatures_applied, dict):
                ...
                if has_sigs:
                    st.success(f"✅ Signatures detected on {len([k for k, v in st.session_state.signatures_applied.items() if v])} page(s)!")
                    ...
                    st.warning("⚠️ No signatures detected. Please drag signatures onto the PDF and click 'Apply Signatures' first.")
            else:
                st.info("💡 Please click the green '✅ Apply Signatures' button in the sidebar after dragging your signature.")
        else:
            st.info("💡 After applying signatures, click the '🔄 Ready to Download?' button above.")
